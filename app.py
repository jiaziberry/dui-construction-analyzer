# -*- coding: utf-8 -*-
"""
对-Construction Analyser
==========================
A Streamlit web application for analysing Chinese 对-constructions.
...
# Page configuration
st.set_page_config(
    page_title="对-Construction Analyser",
    page_icon="🇨🇳",
    layout="wide",
...
def main():
    ...
    # Sidebar
    with st.sidebar:
        st.title("🇨🇳 对-Construction Analyser")
        st.markdown("---")
        
        page = st.radio(
            "Navigation",
            ["🔍 Analyse Sentence", "📚 Construction Guide", "📊 Corpus Statistics", 
             "❓ MS vs ABT", "❓ MS vs DISP", "❓ SI vs DA", "❓ ABT vs SI"]
        )
        
        st.markdown("---")
        st.markdown("""
        ### About
        This tool analyses Chinese 对-constructions based on:
...
def show_analyser_page():
    """Display the sentence analyser page."""
    st.title("🔍 Analyse a 对-Construction")
    
    st.markdown("""
    Enter a Chinese sentence containing **对** to analyse its construction type.
    The analyser will identify:
    ...
    # Analysis results
    if analyse_button and sentence:
    ...
    with col2:
        ...
        <div class="construction-box abt-box">
            ...
            <ul>
                ...
                <li>Activity verbs (commenting, analysing)</li>
...
def show_ms_disp_page():
    """Display MS vs DISP distinction page."""
    st.title("❓ MS vs DISP: Internal State vs Observable Manner")
    
    st.markdown("""
    The distinction between **Mental-State (MS)** and **Disposition (DISP)** is about 
    whether something is **internal** (in X's mind) or **observable** (external behaviour).
    
    The key question to ask is:
    
    > **Can you SEE it, or is it IN X's mind?**
    """)
    ...
    with col1:
        ...
        <div class="construction-box ms-box">
            ...
            <ul>
                <li>Internal psychological state</li>
                <li>NOT observable from outside</li>
                <li>What X thinks/feels about Y</li>
                <li>Psychological state verbs</li>
                <li>Y is stimulus that triggers the state</li>
            </ul>
        </div>
        """, unsafe_allow_html=True)
    ...
    with col2:
        st.markdown("""
        <div class="construction-box disp-box">
            <h3 style="color: #E91E63;">DISP: Disposition</h3>
            <p><strong>X behaves [manner] TOWARD Y</strong></p>
            <ul>
                <li>External observable behaviour</li>
                <li>Observable manner/treatment</li>
                <li>HOW X treats/behaves toward Y</li>
                <li>Stative manner predicates</li>
                <li>Y is target of behavioural manner</li>
            </ul>
        </div>
        """, unsafe_allow_html=True)
    ...
    comparison_df = pd.DataFrame(MS_VS_DISP_DISTINCTION['comparison'])
    st.dataframe(comparison_df, hide_index=True, use_container_width=True)
    
    # Examples
    st.markdown("### Example Comparisons")
    
    examples_df = pd.DataFrame(MS_VS_DISP_DISTINCTION['examples'])
    ...
    for _, row in examples_df.iterrows():
        ...
        with col1:
            st.markdown(f"**{row['verb']}**")
        with col2:
            color = "#9C27B0" if row['type'] == 'MS' else "#E91E63"
            st.markdown(f"<span style='color: {color}; font-weight: bold;'>{row['type']}</span>", 
                       unsafe_allow_html=True)
        with col3:
            st.markdown(row['reason'])
    ...
    test_sentences = MS_VS_DISP_DISTINCTION['test_sentences']
    ...
    for i, (sent, correct, explanation) in enumerate(test_sentences):
        with st.expander(f"Test {i+1}: {sent}"):
            answer = st.radio(
                "What type is this?",
                ["MS", "DISP"],
                key=f"ms_disp_test_{i}",
                horizontal=True
            )
            
            if st.button("Check Answer", key=f"ms_disp_check_{i}"):
                if answer == correct:
                    st.success(f"✅ Correct! {explanation}")
                else:
                    st.error(f"❌ Incorrect. The answer is {correct}. {explanation}")
...
def show_si_da_page():
    """Display SI vs DA distinction page."""
    st.title("❓ SI vs DA: Intervention vs Direction")
    
    st.markdown("""
    The distinction between **Scoped-Intervention (SI)** and **Directed-Action (DA)** 
    hinges on whether **Y is affected** or just **receives the action**.
    
    The key test is:
    
    > **Can the verb take 他 as direct object? (V他✓ or V他✗)**
    """)
    ...
    with col1:
        st.markdown("""
        <div class="construction-box si-box">
            <h3 style="color: #2196F3;">SI: Scoped-Intervention</h3>
            <p><strong>V他✓ - Y is affected</strong></p>
            <ul>
                <li>帮助他✓, 保护他✓, 原谅他✓</li>
                <li>Y undergoes change (被帮助, 被保护)</li>
                <li>对 ≈ "upon / on / over"</li>
                <li>Institutional, procedural context</li>
                <li>Y is bounded scope/patient</li>
            </ul>
        </div>
        """, unsafe_allow_html=True)
    ...
    with col2:
        st.markdown("""
        <div class="construction-box da-box">
            <h3 style="color: #4CAF50;">DA: Directed-Action</h3>
            <p><strong>V他✗ - Y receives but not transformed</strong></p>
            <ul>
                <li>说他✗, 笑他✗, 点头他✗</li>
                <li>Y receives action but not changed</li>
                <li>对 ≈ "toward / to"</li>
                <li>Speech acts, gestures, display</li>
                <li>Y is recipient/addressee</li>
            </ul>
        </div>
        """, unsafe_allow_html=True)
    
    # V他 Test explanation
    st.markdown("### 🔑 The V他 Test")
    st.markdown("""
    | Test | Result | Meaning | Construction |
    |------|--------|---------|--------------|
    | 帮助他 | ✓ 可以说 | Y can be direct object → Y affected | **SI** |
    | 说他 | ✗ 不能说 | Y cannot be direct object → Y is direction only | **DA** |
    | 保护他 | ✓ 可以说 | Y can be direct object → Y affected | **SI** |
    | 笑他 | ✗ 不能说 | Y cannot be direct object → Y is direction only | **DA** |
    """)
    
    # Feature comparison table
    st.markdown("### Feature Comparison")
    
    comparison_df = pd.DataFrame(SI_VS_DA_DISTINCTION['comparison'])
    st.dataframe(comparison_df, hide_index=True, use_container_width=True)
    
    # Examples
    st.markdown("### Example Comparisons")
    
    examples_df = pd.DataFrame(SI_VS_DA_DISTINCTION['examples'])
    ...
    for _, row in examples_df.iterrows():
        ...
        with col1:
            st.markdown(f"**{row['verb']}**")
        with col2:
            color = "#2196F3" if row['type'] == 'SI' else "#4CAF50"
            st.markdown(f"<span style='color: {color}; font-weight: bold;'>{row['type']}</span>", 
                       unsafe_allow_html=True)
        with col3:
            st.markdown(row['reason'])
    ...
    test_sentences = SI_VS_DA_DISTINCTION['test_sentences']
    ...
    for i, (sent, correct, explanation) in enumerate(test_sentences):
        with st.expander(f"Test {i+1}: {sent}"):
            ...
            if st.button("Check Answer", key=f"si_da_check_{i}"):
                if answer == correct:
                    st.success(f"✅ Correct! {explanation}")
                else:
                    st.error(f"❌ Incorrect. The answer is {correct}. {explanation}")
...
def show_abt_si_page():
    """Display ABT vs SI distinction page."""
    st.title("❓ ABT vs SI: Topic vs Affected Scope")
    
    st.markdown("""
    The distinction between **Aboutness (ABT)** and **Scoped-Intervention (SI)** 
    is about whether **Y changes** or is just the **topic of discourse**.
    
    The key question to ask is:
    
    > **Does Y CHANGE, or is Y just the TOPIC?**
    """)
    ...
    with col1:
        ...
        <div class="construction-box abt-box">
            <h3 style="color: #FF9800;">ABT: Aboutness</h3>
            <p><strong>Y is topic - NOT affected</strong></p>
            <ul>
                <li>X produces discourse ABOUT Y</li>
                <li>Y is topic/theme (unaffected)</li>
                <li>Output: words, analysis, commentary</li>
                <li>评价, 分析, 讨论, 研究</li>
                <li>Y just the subject matter</li>
            </ul>
        </div>
        """, unsafe_allow_html=True)
    ...
    with col2:
        ...
        <div class="construction-box si-box">
            <h3 style="color: #2196F3;">SI: Scoped-Intervention</h3>
            <p><strong>Y is affected - undergoes change</strong></p>
            <ul>
                <li>X intervenes ON/UPON Y</li>
                <li>Y is scope/patient (affected)</li>
                <li>Output: change in Y's state</li>
                <li>处理, 帮助, 检查, 保护</li>
                <li>Y undergoes treatment (被处理, 被保护)</li>
            </ul>
        </div>
        """, unsafe_allow_html=True)
    
    # Key distinction
    st.markdown("### 🔑 The Key Test: 被 Passivization")
    st.markdown("""
    | Construction | Example | 被 test | Result |
    |--------------|---------|---------|--------|
    | **SI** | 对问题进行**处理** | 问题**被处理**了 ✓ | Y is affected |
    | **ABT** | 对问题进行**分析** | 问题**被分析**了 ? | Y is just topic |
    | **SI** | 对他进行**帮助** | 他**被帮助**了 ✓ | Y is affected |
    | **ABT** | 对他进行**评价** | 他**被评价**了 ? | Y is just topic of discourse |
    
    *Note: While both can technically be passivized, SI implies real change to Y's state, 
    while ABT only means Y was the topic of discourse.*
    """)
    
    # Feature comparison table
    st.markdown("### Feature Comparison")
    
    comparison_df = pd.DataFrame(ABT_VS_SI_DISTINCTION['comparison'])
    st.dataframe(comparison_df, hide_index=True, use_container_width=True)
    
    # Examples
    st.markdown("### Example Comparisons")
    
    examples_df = pd.DataFrame(ABT_VS_SI_DISTINCTION['examples'])
    ...
    for _, row in examples_df.iterrows():
        ...
        with col1:
            st.markdown(f"**{row['verb']}**")
        with col2:
            color = "#FF9800" if row['type'] == 'ABT' else "#2196F3"
            st.markdown(f"<span style='color: {color}; font-weight: bold;'>{row['type']}</span>", 
                       unsafe_allow_html=True)
        with col3:
            st.markdown(row['reason'])
    ...
    test_sentences = ABT_VS_SI_DISTINCTION['test_sentences']
    ...
    for i, (sent, correct, explanation) in enumerate(test_sentences):
        with st.expander(f"Test {i+1}: {sent}"):
            answer = st.radio(
                "What type is this?",
                ["ABT", "SI"],
                key=f"abt_si_test_{i}",
                horizontal=True
            )
            
            if st.button("Check Answer", key=f"abt_si_check_{i}"):
                if answer == correct:
                    st.success(f"✅ Correct! {explanation}")
                else:
                    st.error(f"❌ Incorrect. The answer is {correct}. {explanation}")

...
A web application for analysing Chinese 对-constructions based on Usage-Based Construction Grammar principles.
...
- DISP: Observable behavioural manner
...
- Added comprehensive `_preprocess_sentence()` method to handle:
  - All whitespace (regular spaces, tabs, newlines, Unicode spaces)
  - BCC corpus markers (NR→某人, NS→某地, NT→某机构, NZ→某某)
  - Control characters and zero-width characters
  - Special symbols (●○◎◇◆□■△▲ etc.)
  - Japanese brackets and variant punctuation
...
- 13 preprocessing test cases passing
- Improved handling of edge cases like "这对健康有害" → EVAL
- Handles sentences with irregular spacing: "我 对 他 说" → works correctly
